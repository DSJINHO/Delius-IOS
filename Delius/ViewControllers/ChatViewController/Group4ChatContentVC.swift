...
import UIKit
class Group4ChatContentVC:UIViewController,UITextFieldDelegate{
    var contentView:Group4ChatContentView!
    ...
    private func setup(){
        ...
        contentView = Group4ChatContentView(frame: view.bounds)
        ...
        contentView.messageCollectionView.register(MySoloChatCell.self, forCellWithReuseIdentifier: "MyGroupChatCellid")
        contentView.messageCollectionView.register(PartnerSoloChatCell.self, forCellWithReuseIdentifier: "PartnerGroupChatCellid")
    ...
    func setData(data:SideGroupStruct,myGroup:MyGroupListStruct){
        ...
        if let im = user3.im{
            let url = CreateThumbUrl.mainImgUrl(uid: user3.uid!, imgObjs: im)
            contentView.imgView3.sd_setImage(with: url)
        }
        if let im = user4.im{
            let url = CreateThumbUrl.mainImgUrl(uid: user4.uid!, imgObjs: im)
...
extension Group4ChatContentVC:UICollectionViewDelegate{
    
}
extension Group4ChatContentVC:UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
