//
//  Group4MatchViewController.swift
//  Delius
//
...
import UIKit
class Group4MatchViewController:UIViewController{
    var contentView:Group4MatchView!
    ...
    private func setup(){
        contentView = Group4MatchView(frame: view.bounds)
        ...
        guard let fourthUser = groupData?.memberStruct?.first(where: {$0.uid! == data.uid4!}) else { return }
        ...
        if let im = fourthUser.im{
            if let uid = fourthUser.uid{
                let url = CreateThumbUrl.mainImgUrl(uid: uid, imgObjs: im)
                contentView.backImgView4.sd_setImage(with: url)
            }
        }
